...
class OpusLMMultiModalProcessor(
    ...
    def _cached_apply_hf_processor(
        ...
        # NOTE(v0.28.0): signature changed from (prompt, mm_data_items, ...)
        # to (inputs, timing_ctx); the bypass semantics are unchanged.
    ...
    def _apply_hf_processor(
        ...
        # NOTE(v0.28.0): the old `_hash_mm_items` hook no longer exists;
        # hashes are computed by ProcessorInputs.get_mm_hashes inside
...
class OpusLMForConditionalGeneration(
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # Build a fake Olmo3Config so we can reuse Olmo3Model.
        # NOTE(v0.28.0): the native Olmo2Model was removed upstream
        # ("Olmo2ForCausalLM" now maps to the Transformers backend).
        # Olmo3's decoder block is architecturally identical to OLMo-2
        # (q_norm/k_norm in attention, post_attention + post_feedforward
        # layernorm, no input_layernorm) when every layer uses full
        # attention, so we pin layer_types accordingly — Olmo3Config
        # defaults to sliding-window layers otherwise. transformers 5.x
        # converts the rope_theta kwarg into config.rope_parameters, which
        ...
        olmo3_hf_config = HFOlmo3Config(
            ...
            # OLMo-2 uses full attention on every layer.
            layer_types=["full_attention"] * config.num_hidden_layers,
        )

        # Temporarily patch vllm_config to use the Olmo3Config
        olmo3_vllm_config = vllm_config.with_hf_config(
            olmo3_hf_config, architectures=["Olmo3ForCausalLM"]
        )

        # --- Language model (OLMo-2-7B backbone) ---
        with self._mark_language_model(vllm_config):
            self.model = Olmo3Model(
                vllm_config=olmo3_vllm_config,
    ...
    def embed_input_ids(
        ...
        *,
        ...
        # `self._has_oov_mm_tokens` inside `_embed_text_input_ids`.
    ...
    def load_weights(
        ...
        convert_opuslm_ckpt.py remapping):
