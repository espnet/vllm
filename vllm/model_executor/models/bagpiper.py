...
"""Inference-only Bagpiper model (Qwen3-8B based multimodal speech-language
...
from vllm.transformers_utils.configs.bagpiper import BagpiperConfig
...
def _get_feat_extract_output_lengths(
    ...
    Args:
        ...
        n_window: The audio encoder's ``n_window`` config value.
            Default 100 (Bagpiper).  Qwen3-Omni uses 50.
    """
...
# ---------------------------------------------------------------------------
class _BagpiperProcessor:
    """Lightweight processor for Bagpiper audio input.
...
class BagpiperProcessingInfo(BaseProcessingInfo):
    ...
    def get_data_parser(self) -> MultiModalDataParser:
        ...
        return BagpiperMultiModalDataParser(
    ...
    def _parse_audio_data(
        ...
        if isinstance(data, dict):
            return DictEmbeddingItems(
                ...
                fields_factory=_bagpiper_field_config,
            )
        return super()._parse_audio_data(data)


class BagpiperMultiModalProcessor(
    BaseMultiModalProcessor[BagpiperProcessingInfo],
):
    ...
    def _get_mm_fields_config(
        ...
        return _bagpiper_field_config(hf_inputs)
...
@MULTIMODAL_REGISTRY.register_processor(
    BagpiperMultiModalProcessor,
    info=BagpiperProcessingInfo,
    dummy_inputs=BagpiperDummyInputsBuilder,
)
class BagpiperForConditionalGeneration(
    nn.Module,
    SupportsMultiModal,
    SupportsPP,
):
    """Bagpiper: Qwen3-8B based multimodal speech-language model.
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        config: BagpiperConfig = vllm_config.model_config.hf_config
        self.config = config
    ...
    # Mask construction
    # ------------------------------------------------------------------
    def _build_masks(self, config: BagpiperConfig):
