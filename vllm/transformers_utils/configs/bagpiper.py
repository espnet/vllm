...
"""Configuration classes for Bagpiper (Qwen3-8B based multimodal
speech-language model with 8-stream parallel codec output).

Bagpiper was formerly named SpeechLM; the ``speechlm`` model_type is kept
as a legacy alias in the config registry.
"""
...
class BagpiperAudioEncoderConfig(PretrainedConfig):
    """Configuration for the Bagpiper audio encoder (Qwen3-Omni Audio Tower).
    ...
    model_type = "bagpiper_audio_encoder"
...
class BagpiperTextConfig(PretrainedConfig):
    """Configuration for the Bagpiper text backbone (Qwen3-8B-Base).
...
class BagpiperConfig(PretrainedConfig):
    """Configuration for BagpiperForConditionalGeneration.

    This is the top-level config that wraps audio_config and text_config.
    It also stores Bagpiper-specific parameters like multi-stream settings,
    ...
    model_type = "bagpiper"
    sub_configs = {
        "audio_config": BagpiperAudioEncoderConfig,
        "text_config": BagpiperTextConfig,
    }
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Sub-configs must be assigned before super().__init__(): transformers
        # v5 runs strict validation (validate_token_ids) inside
        # PretrainedConfig.__init__, which calls self.get_text_config().
        if isinstance(audio_config, dict):
            audio_config = BagpiperAudioEncoderConfig(**audio_config)
        elif audio_config is None:
            audio_config = BagpiperAudioEncoderConfig()
        self.audio_config = audio_config

        if isinstance(text_config, dict):
            text_config = BagpiperTextConfig(**text_config)
